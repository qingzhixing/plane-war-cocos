...
- 脚本：`assets/scripts/`  
  ...
  - 敌人：`EnemyBasic.ts`（冲锋机）；`EnemyTurret.ts`（炮台机 Basic02，锚定高度 + 前摇扇形弹）；`EnemyElite.ts`（Elite01，圆环敌弹）；`EnemyBoss.ts`（第 `BOSS_WAVE` 波占位 Boss，高 HP、慢速、发敌弹）；**敌弹**见 `EnemyBullet.ts`、`enemyBulletFactory.ts`、`EnemyBulletRegistry.ts`；**节点生成**见 `enemyBasicFactory.ts` / `enemyTurretFactory.ts` / `enemyEliteFactory.ts` / `enemyBossFactory.ts`（`EnemySpawner`：**精英率** → 否则 **炮台率** → 否则冲锋机）  
...
- **下一里程碑**：美术资源按 `11_art_and_assets.md` 接入。（**炮台机 Basic02**、**精英率 / Elite01**、**主菜单成绩查询遮罩**、**局内 DPS / `bestDps`**、**敌弹/机体威胁乘区**、**本地成绩**、**续战块刷怪**、**Boss**、**护盾**、**Boss HUD** 等已实现；**连击 / 敌弹 / 撞机**：见既有模块。）
