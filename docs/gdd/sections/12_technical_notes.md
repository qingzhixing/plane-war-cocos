...
- 脚本：`assets/scripts/`  
  ...
  - 全局引用：`battleAccess.ts`（`getBattleMain()`，供 `EnemyBasic` 击杀上报 **`onEnemyKill`**）  
...
- **下一里程碑**：**Boss、玩家受击断连、完整 `main.gd` 统计/本地成绩**；美术资源按 `11_art_and_assets.md` 接入。（**连击与击杀得分区间**：已在 Cocos MVP 落地，见 `comboScore.ts` / `BattleRunState`。）
