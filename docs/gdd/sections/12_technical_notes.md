...
- **下一里程碑**：**精英率**、**小怪机体 `1.12^tier` 等**、**局内 DPS 与 `bestDps` 写入**、**成绩查询面板**；美术资源按 `11_art_and_assets.md` 接入。（**敌弹 speed×`min(1.35,1.04^tier)`**、**本地成绩**、**续战块刷怪**、**Boss**、**护盾**、**Boss HUD** 等已实现；**连击 / 敌弹 / 撞机**：见既有模块。）
