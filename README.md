...
- **玩法扩展**：受击/里程碑音效、Combo 字缩放动画、近纪录阈值提示等 GDD 余项；可选 `npm test` 持续覆盖纯逻辑模块。
